...
frappe.ui.form.on("Vendor Invoice Claim", {
    ...
    vendor: function(frm) {
        if (frm.doc.vendor) {
            frappe.call({
                method: "ap_automation.controllers.vendor_invoice_claim.get_vendor_details",
                args: { vendor: frm.doc.vendor },
                callback: function(r) {
                    if (r.message) {
                        frm.set_value("vendor_name", r.message.vendor_name);
                        frm.set_value("vendor_gstin", r.message.vendor_gstin);
                        frm.set_value("bank_name", r.message.bank_name);
                        frm.set_value("bank_account_number", r.message.bank_account_no);

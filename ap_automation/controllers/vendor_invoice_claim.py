...
class VendorInvoiceClaim(APDocument):
    ...
    def validate_intake_route_and_proofs(self):
        """Enforces dual-route mandatory requirements and audit proof attachments."""
        route = getattr(self, "invoice_type", "With Purchase Order")
    ...
    def calculate_financial_totals(self):
        """Calculates GST amount and Total Invoice amount."""
        base = float(getattr(self, "base_amount", 0.0) or 0.0)
        if base <= 0:
            raise APValidationError("Base Taxable Amount must be strictly positive.")

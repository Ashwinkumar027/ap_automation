...
class EmployeeReimbursementClaim(APDocument):
    ...
    def validate_expense_lines(self):
        """Enforces line-level policy rules: receipts, GSTIN, and cross-lane duplicate spend detection."""
        ...
        for idx, row in enumerate(lines, start=1):
            ...
            if not getattr(row, "receipt_attachment", None):
                raise APValidationError(
                    f"Row {idx} ({merchant} - INR {amt:,.2f}): "
                    "Bill / Tax Receipt attachment is strictly mandatory for every reimbursement line."
                )
